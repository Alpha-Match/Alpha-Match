// Frontend/Front-Server/src/components/dashboard/MainDashboard.tsx
...
import {useHydrated} from '@/core/client/hooks/ui/useHydrated';
import {GET_TOP_COMPANIES} from '@/core/client/services/api/queries/dashboard';
...
export default function MainDashboard({ userMode, activeColor, onSearchClick }: DefaultDashboardProps) {
    ...
    const totalSkills = dashboardData.reduce((acc: number, category: DashboardCategory) => acc + category.skills.length, 0);
    const totalJobs = dashboardData.reduce((acc: number, category: DashboardCategory) => acc + category.skills.reduce((sAcc: number, s: DashboardSkill) => sAcc + s.count, 0), 0);
    ...
    const categoryTotals = dashboardData
        .map((cat: DashboardCategory) => ({
            name: cat.category,
            value: cat.skills.reduce((acc: number, skill: DashboardSkill) => acc + skill.count, 0),
        }))
        .sort((a: { value: number }, b: { value: number }) => b.value - a.value);
    const categoryColorScale: (value: number) => chroma.Color = chroma.scale([baseColor, chroma(baseColor).brighten(2)]).domain([0, categoryTotals.length - 1]);

    const topCompaniesData = (companiesData?.topCompanies || []).map((company: CompanyJobCount) => ({
        name: company.companyName,
        value: company.jobCount,
    })).sort((a: { value: number }, b: { value: number }) => b.value - a.value);

    const allSkills = dashboardData.flatMap((category: DashboardCategory) => category.skills)
        .sort((a: DashboardSkill, b: DashboardSkill) => b.count - a.count)
        .slice(0, 20);
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                             <GenericTreemap
                                ...
                                data={allSkills.map((s: { skill: string; count: number }) => ({ name: s.skill, value: s.count }))}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                            {categoryTotals.map((sortedEntry: { name: string; value: number }, index: number) => {
                                const categoryData = dashboardData.find((d: DashboardCategory) => d.category === sortedEntry.name);
                                ...
                                return (
                                    <GenericTreemap
                                        ...
                                        data={categoryData.skills.map((s: { skill: string; count: number }) => ({ name: s.skill, value: s.count }))}
