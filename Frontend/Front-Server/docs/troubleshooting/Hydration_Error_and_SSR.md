# 트러블슈팅: Next.js Hydration 에러

**작성일**: 2026-01-06
**에러**: `Hydration failed because the server rendered HTML didn't match the client.`
**발생 위치**: `MainDashboard.tsx`

---

## 1. Hydration이란 무엇인가?

Next.js와 같은 서버 사이드 렌더링(SSR) 프레임워크에서 **Hydration(수화, 水化)**은 매우 중요한 개념입니다.

1.  **서버 렌더링 (SSR)**: 사용자가 페이지에 처음 접속하면, 서버는 페이지의 초기 HTML을 미리 만들어서 브라우저에 보냅니다. 덕분에 사용자는 콘텐츠를 매우 빠르게 볼 수 있습니다. (빠른 FCP)
2.  **정적 HTML**: 하지만 서버가 보낸 HTML은 '죽은' 상태입니다. 버튼을 클릭해도 아무 반응이 없고, 상태 변경도 일어나지 않습니다.
3.  **Hydration**: 브라우저가 이 정적 HTML과 JavaScript 코드를 모두 다운로드하면, React가 정적 HTML 위에 '물을 부어' 생명을 불어넣는 과정을 시작합니다. 즉, 이벤트 핸들러(예: `onClick`)를 연결하고, 컴포넌트의 상태를 초기화하여 페이지를 완전히 상호작용 가능한 상태로 만듭니다.

이 과정이 성공하려면, **서버가 렌더링한 HTML**과 **클라이언트에서 첫 렌더링 시 생성한 HTML**이 **완벽하게 일치**해야 합니다.

## 2. 에러 발생 원인

Hydration 에러는 서버와 클라이언트의 첫 렌더링 결과물이 다를 때 발생합니다. `MainDashboard.tsx`에서 발생한 문제의 원인은 다음과 같습니다.

### 상황 분석

-   `MainDashboard` 컴포넌트는 Redux 스토어의 `dashboardData` 상태에 따라 다른 UI를 보여줍니다.
    -   `dashboardData`가 `null`이면: **스켈레톤 UI**를 렌더링
    -   `dashboardData`가 존재하면: **실제 대시보드(파이 차트 등)**를 렌더링

### 렌더링 경로별 차이점

#### 🛤️ 경로 1: 서버 (SSR)
1. 사용자가 페이지에 접속합니다.
2. 서버에서 `HomePage` (Server Component)가 렌더링되고, `initialDashboardData`를 가져옵니다.
3. 이 데이터는 `HomePage.client.tsx`에 props로 전달됩니다.
4. 서버 렌더링 과정에서, Client Component인 `HomePage.client.tsx`의 **`useEffect`는 실행되지 않습니다.**
5. 따라서 Redux 스토어는 `dashboardData`로 채워지지 않고, 초기 상태인 `null`을 유지합니다.
6. `MainDashboard`가 렌더링될 때 `useAppSelector`는 `null`을 반환합니다.
7. **결과**: 서버는 항상 **스켈레톤 UI**가 포함된 HTML을 생성하여 브라우저에 보냅니다.

#### 🛤️ 경로 2: 클라이언트 (Hydration)
1. 브라우저는 서버로부터 스켈레톤 UI가 담긴 HTML을 받습니다.
2. React는 Hydration을 시작하며, 서버가 했던 것과 똑같은 UI를 만들어보려고 시도합니다.
3. 이 때, 사용자가 이전에 다른 페이지를 방문했다가 다시 돌아온 경우, **클라이언트의 Redux 스토어에는 `dashboardData`가 이미 존재할 수 있습니다.**
4. `MainDashboard`가 렌더링될 때 `useAppSelector`는 Redux에서 `dashboardData`를 즉시 가져옵니다.
5. **결과**: 클라이언트는 첫 렌더링부터 **실제 대시보드 UI**를 생성하려고 시도합니다.

### 💥 불일치 발생!

-   **서버가 보낸 것**: `<Skeleton />`
-   **클라이언트가 그리려는 것**: `<SearchedSkillsCategoryDistributionChart />`

React는 이 둘의 구조가 달라 어디에 이벤트 핸들러를 붙여야 할지 알 수 없게 되고, 결국 "Hydration failed" 에러를 발생시킵니다.

## 3. 해결 방안: `useHydrated` 훅

이 문제의 핵심은 "클라이언트의 첫 렌더링이 서버와 무조건 같도록 보장하는 것"입니다. 이를 위해, 컴포넌트가 클라이언트에서 성공적으로 Hydration 되었는지 여부를 알려주는 `useHydrated` 훅을 사용합니다.

### 훅 생성
**파일**: `src/hooks/useHydrated.ts`
```typescript
import { useState, useEffect } from 'react';

export const useHydrated = () => {
  const [isHydrated, setIsHydrated] = useState(false);

  useEffect(() => {
    // useEffect는 클라이언트에서만, 그리고 마운트(하이드레이션) 이후에 실행됩니다.
    setIsHydrated(true);
  }, []);

  return isHydrated;
};
```

### 훅 적용
**파일**: `src/components/dashboard/MainDashboard.tsx`
```typescript
import { useHydrated } from '@/hooks/useHydrated'; // 훅 임포트

export default function MainDashboard() {
    const isHydrated = useHydrated(); // 하이드레이션 상태 추적
    const userMode = useAppSelector((state) => state.ui.userMode);
    const dashboardData = useAppSelector((state) => state.search[userMode].dashboardData);

    // ...

    // 하이드레이션 전이거나 데이터가 아직 로드되지 않은 경우 스켈레톤을 표시합니다.
    if (!isHydrated || !dashboardData) {
        return (
            // 스켈레톤 UI
        );
    }
    
    // ... 실제 대시보드 UI
}
```

### 해결 원리

1.  **서버 렌더링**: `useHydrated`는 서버에서 `false`를 반환합니다. `if (!isHydrated ...)` 조건이 `true`가 되어 **스켈레톤 UI**가 렌더링됩니다.
2.  **클라이언트 첫 렌더링 (Hydration)**: `useState(false)`에 의해 `isHydrated`는 `false`입니다. `if (!isHydrated ...)` 조건이 `true`가 되어 **스켈레톤 UI**가 렌더링됩니다.
3.  **✅ 일치**: 서버와 클라이언트의 결과가 완벽히 일치하여 Hydration이 성공적으로 완료됩니다.
4.  **클라이언트 두 번째 렌더링**: Hydration 직후, `useHydrated` 훅의 `useEffect`가 실행되어 `isHydrated` 상태가 `true`로 변경됩니다. 이로 인해 컴포넌트가 리렌더링됩니다.
5.  이제 `!isHydrated` 조건이 `false`가 되고, Redux 스토어에 `dashboardData`가 있다면 **실제 대시보드 UI**가 화면에 그려집니다.

이 패턴은 Hydration 오류를 해결하는 표준적이고 안정적인 방법입니다.

## 4. 기타 Hydration 에러 발생 원인

-   `new Date()`, `Math.random()`과 같이 서버와 클라이언트에서 결과가 달라질 수 있는 코드 사용
-   `window`, `localStorage` 등 브라우저 전용 API를 조건문 없이 사용
-   CSS-in-JS 라이브러리의 서버 렌더링 설정 오류
-   쿼리 파라미터나 쿠키 값에 따라 UI가 달라지는데, 서버와 클라이언트의 접근 방식이 다른 경우
