...
`redux-persist` 라이브러리를 사용하여 Redux 스토어의 특정 상태를 브라우저의 로컬 스토리지에 영구적으로 저장하고, 멀티 탭 환경에서는 선택적으로 동기화합니다.

*   **영속화 대상**: `ui` (UI 네비게이션, 히스토리 등), `search` (검색 조건 및 결과), `theme` (테마 설정).
*   **멀티 탭 동기화 전략**:
    ...
    *   **`ui`**: 이 슬라이스는 `redux-persist`를 통해 각 탭에서 독립적으로 영속화되지만, 멀티 탭 동기화 로직에서는 제외됩니다. 즉, 다른 탭에서 `ui` 상태가 변경되더라도 현재 탭의 `ui` 상태를 덮어쓰지 않으므로, 각 탭의 고유한 UI 흐름(예: `pageViewMode`, `selectedMatchId`)이 유지됩니다.
*   **사용자 경험**: 앱 재시작 및 멀티 탭 환경에서도 검색 조건, 검색 결과, 테마 설정 등은 탭 간에 일관되게 유지되거나 동기화되며, UI 네비게이션 흐름은 각 탭의 맥락에 따라 독립적으로 보존되어 최적화된 사용자 경험을 제공합니다.
