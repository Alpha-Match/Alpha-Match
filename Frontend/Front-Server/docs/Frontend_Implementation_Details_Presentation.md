...
*   **영속화 대상**: `search` (검색 조건 및 결과), `theme` (테마 설정).
    *   `ui` 관련 상태(예: `userMode`, `pageViewMode`, `selectedMatchId`)는 각 탭에서 완전히 독립적으로 관리되며, 새로고침 시 초기 상태로 재설정됩니다. (영속화하지 않음)
*   **멀티 탭 동기화 전략**:
    ...
    *   **`ui`**: 이 슬라이스는 `redux-persist`에서 완전히 제외되므로, 각 탭에서 독립적으로 초기 상태로 시작합니다.
*   **사용자 경험**: 앱 재시작 및 멀티 탭 환경에서도 검색 조건, 검색 결과, 테마 설정 등은 탭 간에 일관되게 유지되거나 동기화되며, UI 네비게이션 흐름(`userMode` 포함)은 각 탭의 맥락에 따라 항상 초기 상태로 독립적으로 보존됩니다.
