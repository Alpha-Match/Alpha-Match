...
*   **Fetch Policy**: Apollo Client는 `fetchPolicy` 설정을 통해 캐시와 네트워크 요청 간의 우선순위를 제어합니다.
    *   **기본 설정**: `apollo-client.ts`에서는 `watchQuery`에 `cache-and-network`를, 일반 `query`에는 `network-only`를 기본으로 설정했습니다. `network-only` 정책은 캐시에 데이터가 있더라도 항상 네트워크 요청을 강제하여 데이터의 최신성을 보장하지만, 불필요한 로딩 지연을 발생시킬 수 있습니다.
    *   **최적화**: `SearchResultAnalysisPanel`, `MainDashboard`, `TopSkills`, `useSearchMatches` 등 특정 컴포넌트에서는 사용자 경험 최적화를 위해 `fetchPolicy: 'cache-first'`를 명시적으로 적용했습니다. 이는 캐시에 유효한 데이터가 있을 경우 즉시 이를 반환하여 로딩 스피너의 불필요한 노출을 줄이고, 사용자에게 더 빠른 반응성을 제공합니다. `useMatchDetail`과 같은 상세 정보 조회 훅에서도 이미 `cache-first`를 사용하고 있었습니다.
    *   **동시 로딩 최적화**: `HomePage.client.tsx`에서 검색 시작 시 `searchedSkills`를 Redux에 즉시 디스패치하도록 변경하여, `SearchResultAnalysisPanel`이 메인 검색 결과 쿼리와 병렬로 데이터를 가져올 수 있도록 개선했습니다. 이로써 분석 패널의 스피너가 메인 검색 결과 이후에 시작되는 듯한 "동기적" 느낌을 해소하고, 사용자 경험을 향상시켰습니다.

### 2.4 Redux-persist를 활용한 상태 저장 및 멀티 탭 동기화

`redux-persist` 라이브러리를 사용하여 Redux 스토어의 특정 상태를 브라우저의 로컬/세션 스토리지에 영구적으로 저장하고, 멀티 탭 환경에서는 선택적으로 동기화합니다.

*   **영속화 대상 및 전략**:
    *   **`search` 및 `theme`**: `localStorage`를 사용하여 브라우저 재시작 후에도 영구적으로 저장되고, 탭 간에 동기화됩니다. (`Frontend/Front-Server/src/app/providers.tsx`의 `storage` 이벤트 리스너를 통해 동기화).
    *   **`ui`**: `sessionStorage`를 사용하여 해당 탭/창이 닫히기 전까지는 새로고침 시에도 상태가 유지되지만, 브라우저 재시작 시에는 초기화됩니다. (세션별 관리)
*   **멀티 탭 동기화**:
    *   **`search` 및 `theme`**: 이 슬라이스들은 탭 간에 동기화되어 일관된 검색 경험과 사용자 테마 설정을 제공합니다. `Frontend/Front-Server/src/app/providers.tsx`에 구현된 `window.addEventListener('storage', ...)`를 통해 `localStorage` 변경 이벤트를 감지하고, 다른 탭에서 변경이 발생하면 해당 슬라이스만 재수화됩니다.
    *   **`ui`**: `sessionStorage`를 사용하므로 탭 간에 자동으로 독립적으로 관리되며, 멀티 탭 동기화 로직에 포함되지 않습니다.
*   **사용자 경험**: 앱 재시작 후에는 UI 네비게이션 흐름(`userMode` 포함)이 초기 상태로 돌아가지만, 세션이 유지되는 동안에는 탭 내에서 새로고침해도 이전 상태가 유지됩니다. 검색 조건, 검색 결과, 테마 설정 등은 탭 간에 일관되게 유지되거나 동기화되어 최적화된 사용자 경험을 제공합니다.
...
### 4.1 Apollo Client 캐싱 및 스피너 문제

프론트엔드 애플리케이션에서 특정 화면(예: `SearchResultAnalysisPanel`)을 다시 방문했을 때, 이전에 데이터를 성공적으로 불러왔음에도 불구하고 로딩 스피너가 항상 표시되는 현상이 발생했습니다. ViewModel 패턴과 `redux-persist`를 통해 상태 관리를 하고 있음에도 불구하고 데이터가 즉시 로드되지 않는 문제였습니다.

*   **원인 분석**: 문제의 근본 원인은 Apollo Client의 `fetchPolicy` 설정에 있었습니다. `apollo-client.ts`의 전역 `defaultOptions`에서 `query`의 `fetchPolicy`가 `network-only`로 설정되어 있었고, `useQuery` 훅은 이를 따르고 있었습니다. 이 정책은 캐시에 유효한 데이터가 있더라도 항상 네트워크 요청을 강제하기 때문에 불필요한 로딩 스피너가 노출되었습니다. 또한, `SearchResultAnalysisPanel`의 쿼리가 메인 검색 쿼리(`useSearchMatches`) 완료 후 Redux `searchedSkills` 상태 업데이트를 기다렸다가 시작되는 데이터 종속성으로 인해 스피너가 순차적으로 나타나는 문제가 있었습니다.

*   **해결책**:
    1.  각 컴포넌트(`SearchResultAnalysisPanel`, `MainDashboard`, `TopSkills`, `useSearchMatches`)의 `useQuery` 훅에서 `fetchPolicy`를 `cache-first`로 명시적으로 설정하여, Apollo Client 캐시의 데이터를 우선적으로 사용하도록 변경했습니다. 이는 동일한 검색 조건으로 화면을 다시 방문했을 때 캐시 데이터가 즉시 표시되도록 합니다.
    2.  `HomePage.client.tsx`의 `handleSearch` 함수에서 검색 시작 시 `selectedSkills`를 Redux의 `searchedSkills`로 즉시 디스패치하도록 변경했습니다. 이로써 `SearchResultAnalysisPanel`의 쿼리가 메인 검색 쿼리와 병렬로 시작될 수 있게 되어, 로딩 경험이 더 동시적으로 느껴지게 개선되었습니다.

### 4.2 Hydration 오류 처리

*   **문제**: Next.js 환경에서 Server Component와 Client Component 간의 렌더링 불일치(HTML 구조, 속성 등)로 인해 Hydration 과정에서 오류가 발생할 수 있습니다. 이는 특히 브라우저 환경에 따라 다르게 렌더링되는 요소(예: `<canvas>`, 날짜/시간 포맷)나 초기 클라이언트 상태에 따라 UI가 변경되는 경우 발생하기 쉽습니다. 프로젝트에서도 이와 유사한 렌더링 불일치 문제가 발생할 가능성이 있었고, 이를 선제적으로 방지할 필요가 있었습니다.
*   **해결**: `src/lib/client/hooks/ui/useHydrated.ts`의 `useHydrated` 훅을 도입했습니다. 이 훅은 컴포넌트가 클라이언트에서 완전히 하이드레이션된 후에만 특정 클라이언트 전용 로직이나 UI 렌더링을 시작하도록 하여 렌더링 불일치로 인한 오류를 방지합니다. 예를 들어, 클라이언트 전용 라이브러리(차트, 맵 등)를 사용하는 컴포넌트는 `useHydrated`를 통해 안전하게 마운트될 수 있습니다.
