# 🔐 동시성 제어 가이드

**작성일:** 2025-12-17 (업데이트)
**대상:** Batch-Server
**목적:** Race Condition 대응 및 동시성 제어 전략

---

## 동시성 이슈 발생 지점

Batch Server에서 동시성 문제가 발생할 수 있는 주요 지점:

1. **캐시 무효화 중복 호출**
2. **Checkpoint 업데이트 경쟁**
3. **Vector Upsert와 Metadata Upsert 경쟁**
4. **Python Stream 비정상 종료 시 Checkpoint 경쟁**

---

## 1. 캐시 무효화 중복 호출 방지

### 문제 상황

```java
// Bad: 동시에 여러 스레드가 캐시 무효화 호출
chunk1 처리 완료 → API Server 캐시 무효화
chunk2 처리 완료 → API Server 캐시 무효화  (중복!)
chunk3 처리 완료 → API Server 캐시 무효화  (중복!)
```

### 해결책: AtomicBoolean

```java
@Component
@Slf4j
public class CacheInvalidateGrpcClient {

    private final AtomicBoolean invalidating = new AtomicBoolean(false);

    public Mono<Boolean> invalidateCache(String target) {
        return Mono.defer(() -> {
            // CAS (Compare-And-Set) 연산으로 중복 방지
            if (!invalidating.compareAndSet(false, true)) {
                log.warn("Cache invalidation already in progress, skipping");
                return Mono.just(false);
            }

            try {
                log.info("Sending cache invalidation request for target: {}", target);

                CacheInvalidateRequest request = CacheInvalidateRequest.newBuilder()
                        .setTarget(target)
                        .build();

                CacheInvalidateResponse response = blockingStub
                        .withDeadlineAfter(10, TimeUnit.SECONDS)
                        .invalidateCache(request);

                return Mono.just(response.getSuccess());

            } catch (Exception e) {
                log.error("Error during cache invalidation", e);
                return Mono.error(e);
            } finally {
                invalidating.set(false);  // 반드시 해제
            }
        });
    }
}
```

### AtomicBoolean vs synchronized

| 방식 | 장점 | 단점 |
|-----|------|------|
| **AtomicBoolean** | Lock-free, 빠름 | Boolean만 가능 |
| **synchronized** | 복잡한 로직 보호 가능 | Blocking, Virtual Thread Pinning |
| **ReentrantLock** | 유연한 제어 | 코드 복잡도 증가 |

---

## 2. Checkpoint 업데이트 경쟁 방지

### 문제 상황

```
Thread 1: chunk1 처리 → checkpoint = uuid_1
Thread 2: chunk2 처리 → checkpoint = uuid_2
Thread 1: checkpoint 업데이트 (uuid_1) ← 나중에 실행되면 문제!
```

### 해결책 1: Spring Batch 순차 처리 (권장)

**Spring Batch는 기본적으로 순차 처리를 보장합니다:**

```java
// ItemReader가 순차적으로 Item을 반환
@Component
public class RecruitItemReader extends DomainItemReader<RecruitRow> {
    @Override
    public RecruitRow read() throws Exception {
        // BlockingQueue에서 순차적으로 1개씩 반환
        T item = queue.poll(100, TimeUnit.MILLISECONDS);
        if (item == null && streamCompleted && queue.isEmpty()) {
            return null;
        }
        return item;
    }
}

// ItemWriter에서 Checkpoint 업데이트
@Component
public class DomainItemWriter<M, E> implements ItemWriter<DomainItem<M, E>> {
    @Override
    @Transactional
    public void write(Chunk<? extends DomainItem<M, E>> chunk) {
        // 1. Metadata/Embedding 저장
        metadataRepository.upsertAll(metadataList);
        embeddingRepository.upsertAll(embeddingList);

        // 2. Checkpoint 업데이트 (트랜잭션 내)
        UUID lastId = chunk.getItems().get(chunk.size() - 1).getMetadata().getId();
        checkpointRepository.updateLatestCheckpoint(lastId);
    }
}
```

**장점**:
- Spring Batch가 순서를 보장
- Chunk 단위 트랜잭션으로 Checkpoint 업데이트 원자성 보장
- 간단하고 안전함

### 해결책 2: DB 트랜잭션 + 낙관적 락

```java
@Repository
public interface CheckpointRepository extends JpaRepository<CheckpointEntity, Long> {

    @Lock(LockModeType.OPTIMISTIC)
    @Query("SELECT c FROM CheckpointEntity c WHERE c.id = 1")
    Optional<CheckpointEntity> findLatestForUpdate();
}
```

```java
@Entity
@Table(name = "embedding_batch_checkpoint")
public class CheckpointEntity {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @Column(name = "last_processed_uuid")
    private UUID lastProcessedUuid;

    @Version  // 낙관적 락
    private Long version;

    // ...
}
```

---

## 3. Upsert 순서 보장

### 문제 상황

```java
// FK 제약으로 인해 순서가 중요
embedding.upsert()  // FK 제약 위반! (metadata가 아직 없음)
metadata.upsert()
```

### 해결책 1: Spring Batch Writer에서 순서 보장 (권장)

```java
@Component
public class DomainItemWriter<M, E> implements ItemWriter<DomainItem<M, E>> {

    @Override
    @Transactional
    public void write(Chunk<? extends DomainItem<M, E>> chunk) {
        List<M> metadataList = new ArrayList<>();
        List<E> embeddingList = new ArrayList<>();

        // 1. Chunk에서 데이터 추출
        for (DomainItem<M, E> item : chunk) {
            metadataList.add(item.getMetadata());
            embeddingList.add(item.getEmbedding());
        }

        // 2. Metadata 먼저 저장 (FK 참조 대상)
        metadataRepository.upsertAll(metadataList);

        // 3. Embedding 나중에 저장 (FK 참조)
        embeddingRepository.upsertAll(embeddingList);

        // 4. Checkpoint 업데이트 (트랜잭션 내)
        UUID lastId = metadataList.get(metadataList.size() - 1).getId();
        checkpointRepository.updateLatestCheckpoint(lastId);
    }
}
```

### 해결책 2: Candidate 3-Table Split (복잡한 도메인)

```java
@Component
public class CandidateItemWriter implements ItemWriter<CandidateItem> {

    @Override
    @Transactional
    public void write(Chunk<? extends CandidateItem> chunk) {
        List<CandidateEntity> candidates = new ArrayList<>();
        List<CandidateSkillEntity> allSkills = new ArrayList<>();
        List<CandidateSkillsEmbeddingEntity> embeddings = new ArrayList<>();

        for (CandidateItem item : chunk) {
            candidates.add(item.getCandidate());
            allSkills.addAll(item.getSkills());
            embeddings.add(item.getEmbedding());
        }

        // 순서 보장: candidate → candidate_skill → candidate_skills_embedding
        // 1. candidate 테이블 (PK)
        candidateRepository.upsertAll(candidates);

        // 2. candidate_skill 테이블 (FK → candidate)
        candidateSkillRepository.upsertAll(allSkills);

        // 3. candidate_skills_embedding 테이블 (FK → candidate)
        candidateSkillsEmbeddingRepository.upsertAll(embeddings);
    }
}
```

---

## 4. Idempotent 설계

### Upsert 전략

```sql
-- Idempotent: 같은 요청을 여러 번 실행해도 결과 동일
INSERT INTO recruit_metadata (id, company_name, exp_years, ...)
VALUES (?, ?, ?, ...)
ON CONFLICT (id) DO UPDATE SET
    company_name = EXCLUDED.company_name,
    exp_years = EXCLUDED.exp_years,
    updated_at = NOW();
```

### UUID 기반 병렬 처리의 이점 (NEW)

#### AutoIncrement의 동시성 문제

```java
// Bad: AutoIncrement (시퀀스 락 경쟁)
INSERT INTO recruit_metadata (company_name, ...) VALUES (...);
// Thread 1, 2, 3이 동시에 시퀀스 획득 대기
// → 병목 발생, 처리량 저하

// 병렬 처리 시:
parallel(4).flatMap(chunk -> {
    // 4개 스레드가 모두 시퀀스 락 대기
    // 실제로는 순차 처리와 유사한 성능
})
```

#### UUID 기반 병렬 처리

```java
// Good: UUID (사전 생성, 경합 없음)
@Service
public class ChunkProcessor {

    public void processChunkParallel(RowChunk chunk) {
        List<MetadataEntity> metadataList = chunk.getRowsList().stream()
                .map(row -> MetadataEntity.builder()
                        .id(UUID.fromString(row.getId()))  // Python에서 생성한 UUID 사용
                        .companyName(row.getCompanyName())
                        .build())
                .toList();

        // 병렬 Upsert: 각 스레드가 독립적으로 처리
        metadataRepository.upsertAll(metadataList);
        // UUID이므로 PK 생성 경합 없음!
    }
}
```

#### 성능 비교

| PK 타입 | 순차 처리 | 병렬 처리 (4 threads) | 병렬 효과 |
|---------|----------|----------------------|----------|
| AutoIncrement | 100 rows/sec | 120 rows/sec | ⭐⭐ (20% 향상) |
| UUID v7/ULID | 100 rows/sec | 380 rows/sec | ⭐⭐⭐⭐⭐ (거의 4배) |

**이유:**
- AutoIncrement: 시퀀스 락으로 인해 실질적 병렬도 저하
- UUID: 각 스레드가 완전 독립적으로 작동

#### UUID + 병렬 구독 조합

```java
public Mono<Void> processStreamWithUuidParallel() {
    return grpcClient.streamEmbeddings(null, 300)
            // 청크 재분할
            .flatMap(chunk -> Flux.fromIterable(chunk.getRowsList())
                    .buffer(50)
                    .map(rows -> RowChunk.newBuilder().addAllRows(rows).build())
            )
            // 병렬 처리 (UUID이므로 경합 없음)
            .parallel(4)
            .runOn(jpaScheduler)
            .flatMap(subChunk -> Mono.fromCallable(() -> {
                // 각 스레드가 독립적으로 UUID 사용
                chunkProcessor.processChunk(subChunk);
                return subChunk;
            }))
            .sequential()
            .then();
}
```

### 재시도 안전성

```java
// Idempotent한 작업이므로 재시도 안전
public Mono<Void> processWithRetry() {
    return processChunk(chunk)
            .retry(3)  // 실패 시 3회 재시도
            .onErrorResume(e -> {
                log.error("Failed after retries", e);
                return saveToDLQ(chunk, e);
            });
}
```

---

## 5. DLQ (Dead Letter Queue) 처리

### 실패 레코드 저장

**Spring Batch Writer에서 DLQ 처리:**

```java
@Component
public class DomainItemWriter<M, E> implements ItemWriter<DomainItem<M, E>> {

    @Override
    @Transactional
    public void write(Chunk<? extends DomainItem<M, E>> chunk) {
        List<DomainItem<M, E>> successItems = new ArrayList<>();
        List<DlqEntity> dlqItems = new ArrayList<>();

        // 1. 개별 item 처리 (실패 시 DLQ)
        for (DomainItem<M, E> item : chunk) {
            try {
                validateItem(item);  // 검증 로직
                successItems.add(item);
            } catch (Exception e) {
                log.error("Item validation failed: {}", item.getMetadata().getId(), e);

                // DLQ에 저장
                DlqEntity dlq = DlqEntity.builder()
                        .domain(domain)
                        .recordId(item.getMetadata().getId())
                        .errorMessage(e.getMessage())
                        .payload(toJson(item))
                        .build();
                dlqItems.add(dlq);
            }
        }

        // 2. DLQ 저장
        if (!dlqItems.isEmpty()) {
            dlqRepository.saveAll(dlqItems);
        }

        // 3. 성공한 데이터만 Batch Upsert
        if (!successItems.isEmpty()) {
            List<M> metadataList = successItems.stream()
                    .map(DomainItem::getMetadata)
                    .collect(Collectors.toList());
            List<E> embeddingList = successItems.stream()
                    .map(DomainItem::getEmbedding)
                    .collect(Collectors.toList());

            metadataRepository.upsertAll(metadataList);
            embeddingRepository.upsertAll(embeddingList);
        }
    }
}
```

### DLQ 재처리

```java
@Service
public class DlqReprocessService {

    @Scheduled(cron = "0 0 3 * * ?")  // 매일 새벽 3시
    public void reprocessDLQ() {
        List<DlqEntity> dlqList = dlqRepository.findAllOrderByCreatedAtAsc();

        for (DlqEntity dlq : dlqList) {
            try {
                RecruitRow row = fromJson(dlq.getPayload());
                processRow(row);

                // 성공 시 DLQ에서 삭제
                dlqRepository.delete(dlq);
                log.info("Successfully reprocessed DLQ record: {}", dlq.getId());

            } catch (Exception e) {
                log.error("Failed to reprocess DLQ record: {}", dlq.getId(), e);
                // 실패 시 그대로 유지 (다음 재처리 시도)
            }
        }
    }
}
```

---

## 6. Python Stream 비정상 종료 대응

### Checkpoint 기반 재시작

**Spring Batch는 자동으로 Checkpoint를 지원합니다:**

```java
// 1. ItemReader에서 Checkpoint 재개
@Component
public class RecruitItemReader extends DomainItemReader<RecruitRow> {

    @PostConstruct
    public void init() {
        // Checkpoint에서 마지막 처리 UUID 조회
        UUID lastProcessedUuid = checkpointRepository
                .findByDomain("recruit")
                .map(CheckpointEntity::getLastProcessedUuid)
                .orElse(null);

        log.info("Starting stream from checkpoint: {}", lastProcessedUuid);

        // gRPC Stream 생성 (Checkpoint부터 재개)
        createStream(lastProcessedUuid)
                .doOnNext(queue::offer)
                .doOnComplete(() -> streamCompleted = true)
                .doOnError(e -> log.error("Stream error, will retry from checkpoint", e))
                .subscribeOn(Schedulers.boundedElastic())
                .subscribe();
    }

    @Override
    protected Flux<RecruitRow> createStream(UUID lastProcessedUuid) {
        return grpcClient.streamEmbeddings(lastProcessedUuid, chunkSize)
                .flatMapIterable(RowChunk::getRowsList)
                .map(row -> row.getRecruitChunk())
                .filter(Objects::nonNull);
    }
}

// 2. ItemWriter에서 Checkpoint 업데이트
@Component
public class DomainItemWriter<M, E> implements ItemWriter<DomainItem<M, E>> {

    @Override
    @Transactional
    public void write(Chunk<? extends DomainItem<M, E>> chunk) {
        // ... metadata/embedding 저장

        // Checkpoint 업데이트
        UUID lastId = chunk.getItems().get(chunk.size() - 1).getMetadata().getId();
        checkpointRepository.updateLatestCheckpoint(domain, lastId);
    }
}
```

### Spring Batch 자동 재시작

**Job 실패 시 자동으로 Checkpoint부터 재시작:**

```java
@Configuration
public class BatchJobConfig {

    @Bean
    public Job recruitEmbeddingProcessingJob() {
        return new JobBuilder("recruitEmbeddingProcessingJob", jobRepository)
                .listener(embeddingJobListener)
                .start(recruitStep())
                .build();
    }

    @Bean
    public Step recruitStep() {
        return new StepBuilder("recruitStep", jobRepository)
                .<RecruitRow, DomainItem<RecruitMetadataEntity, RecruitEmbeddingEntity>>chunk(chunkSize, transactionManager)
                .reader(recruitItemReader)
                .processor(recruitItemProcessor)
                .writer(domainItemWriter)
                .faultTolerant()
                .skipLimit(100)
                .skip(Exception.class)
                .listener(embeddingStepListener)
                .build();
    }
}

// Job 재시작 예시
JobParameters jobParameters = new JobParametersBuilder()
        .addString("timestamp", LocalDateTime.now().toString())
        .toJobParameters();

// 실패한 Job 재시작 → Checkpoint부터 자동 재개
jobOperator.restart(jobExecutionId);
```

---

## 7. 동시성 제어 패턴 요약

### 사용 시나리오별 권장 방법

| 시나리오 | 권장 방법 | 코드 예시 |
|---------|----------|----------|
| **간단한 Flag** | AtomicBoolean | `compareAndSet()` |
| **복잡한 로직** | ReentrantLock | `lock.lock()` / `unlock()` |
| **DB 경쟁** | 낙관적 락 | `@Version` + `@Lock` |
| **순서 보장** | Sequential Stream | `.concatMap()` |
| **재시도** | Idempotent + Retry | `ON CONFLICT DO UPDATE` |
| **실패 처리** | DLQ Pattern | `try-catch` + DLQ 저장 |
| **병렬 Insert** | UUID v7/ULID | 시퀀스 경합 제거 |
| **청크 병렬 처리** | Reactive parallel() | `.parallel(4).runOn(scheduler)` |
| **테이블 병렬 쓰기** | Virtual Thread | `Executors.newVirtualThreadPerTaskExecutor()` |

### UUID 기반 병렬 처리 Best Practices

#### 1. Python 서버에서 UUID 생성
```python
# Python 서버에서 UUID v7 생성
import uuid7

recruit_id = uuid7.uuid7()  # 시간순 정렬 가능
```

#### 2. Batch 서버에서 UUID 활용 (Spring Batch Processor)
```java
@Component
public class RecruitItemProcessor
        extends DomainItemProcessor<RecruitRow, RecruitMetadataEntity, RecruitEmbeddingEntity> {

    @Override
    public DomainItem<RecruitMetadataEntity, RecruitEmbeddingEntity> process(RecruitRow item) {
        // Python에서 생성한 UUID 그대로 사용
        UUID recruitId = UUID.fromString(item.getId());

        RecruitMetadataEntity metadata = RecruitMetadataEntity.builder()
                .id(recruitId)  // 변환만 수행, PK 경합 없음
                .companyName(item.getCompanyName())
                .build();

        RecruitEmbeddingEntity embedding = RecruitEmbeddingEntity.builder()
                .id(recruitId)
                .embedding(convertToVector(item.getEmbeddingList()))
                .build();

        return new DomainItem<>(metadata, embedding);
    }
}
```

#### 3. Spring Batch Writer는 기본적으로 순차 처리
```java
@Component
public class DomainItemWriter<M, E> implements ItemWriter<DomainItem<M, E>> {

    @Override
    @Transactional
    public void write(Chunk<? extends DomainItem<M, E>> chunk) {
        // Spring Batch는 Chunk를 순차적으로 처리
        // UUID 기반이므로 PK 경합 없음

        List<M> metadataList = chunk.getItems().stream()
                .map(DomainItem::getMetadata)
                .collect(Collectors.toList());

        List<E> embeddingList = chunk.getItems().stream()
                .map(DomainItem::getEmbedding)
                .collect(Collectors.toList());

        // Batch Upsert: UUID이므로 병렬 Insert 가능 (DB 레벨)
        metadataRepository.upsertAll(metadataList);
        embeddingRepository.upsertAll(embeddingList);

        // Checkpoint 업데이트
        UUID lastId = metadataList.get(metadataList.size() - 1).getId();
        checkpointRepository.updateLatestCheckpoint(domain, lastId);
    }
}
```

**참고:** Spring Batch는 기본적으로 순차 처리를 보장하므로, Reactive parallel() 패턴은 불필요합니다.
UUID의 장점은 **DB 레벨의 병렬 Insert**에서 발휘됩니다 (시퀀스 경합 없음).

---

## 8. Virtual Thread와 동시성

### 8.1 Virtual Thread 병렬 테이블 쓰기 (구현 완료)

**v2 4-table 구조에서 FK 없는 테이블 병렬 처리:**

```
[Sequential - 기존]
시간 ─────────────────────────────────────────────────────────►
     [recruit]───────►[skill]─────►[desc]────►[embedding]────►
     |──── 197ms ────||── 150ms ──||─ 100ms ─||── 200ms ────|
                                                    총: 647ms

[Parallel - Virtual Thread]
시간 ─────────────────────────────────────────────────────►
     [recruit]───────►┌─[skill]───────┐
                      ├─[desc]────────┼──► 동시 완료
                      └─[embedding]───┘
     |──── 197ms ────||──── 200ms ────|
                            (max)     총: 397ms
```

**구현 코드 (RecruitDataProcessor.java):**

```java
@Override
public int processChunk(byte[] jsonChunk) {
    // ... JSON 파싱 및 Entity 변환 ...

    // 1. recruit 먼저 저장 (FK 제약조건)
    recruitRepository.upsertAll(recruitEntities);

    // 2. 나머지 3개 테이블 병렬 처리 (Virtual Thread)
    try (ExecutorService executor = Executors.newVirtualThreadPerTaskExecutor()) {
        Future<?> skillFuture = executor.submit(() ->
            recruitSkillRepository.upsertAll(skillEntities));
        Future<?> descFuture = executor.submit(() ->
            recruitDescriptionRepository.upsertAll(descriptionEntities));
        Future<?> embeddingFuture = executor.submit(() ->
            recruitSkillsEmbeddingRepository.upsertAll(embeddingEntities));

        // 모든 병렬 작업 완료 대기
        skillFuture.get();
        descFuture.get();
        embeddingFuture.get();
    }
    return dtos.size();
}
```

**성능 개선 결과 (Recruit 도메인, 87,488건):**

| 지표 | Baseline | Virtual Thread | 개선율 |
|-----|----------|----------------|--------|
| 소요 시간 | 12m 54.8s | 8m 38.2s | **33.1% 단축** |
| 처리량 | 113.0 rps | 168.8 rps | **49.4% 증가** |

### 8.2 Connection Pool 설정

Virtual Thread 병렬 쓰기 시 HikariCP 설정:

```yaml
spring:
  datasource:
    hikari:
      maximum-pool-size: 20  # Virtual Thread 3개 × 여유분
      minimum-idle: 5
```

**규칙:** `Pool Size > 동시 Virtual Thread 수`

### 8.3 Pinning 방지

```java
// Bad: synchronized (Carrier Thread Pinning)
synchronized(lock) {
    repository.save(entity);
}

// Good: ReentrantLock
ReentrantLock lock = new ReentrantLock();
lock.lock();
try {
    repository.save(entity);
} finally {
    lock.unlock();
}
```

### 8.4 Structured Concurrency (Java 21+)

```java
public void processChunkStructured(RowChunk chunk) throws Exception {
    try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
        Future<Void> metadataTask = scope.fork(() -> {
            metadataRepository.upsertAll(metadataList);
            return null;
        });

        Future<Void> embeddingTask = scope.fork(() -> {
            embeddingRepository.upsertAll(embeddingList);
            return null;
        });

        scope.join();  // 모두 완료 대기
        scope.throwIfFailed();  // 실패 시 예외

        // Checkpoint 업데이트
        updateCheckpoint(chunk);
    }
}
```

---

## 9. 모니터링 및 디버깅

### 동시성 이슈 로깅

```java
@Slf4j
@Component
public class ConcurrencyMonitor {

    private final AtomicInteger activeChunks = new AtomicInteger(0);

    public void chunkStarted() {
        int active = activeChunks.incrementAndGet();
        log.debug("Chunk processing started. Active chunks: {}", active);

        if (active > 5) {
            log.warn("High concurrency detected: {} chunks", active);
        }
    }

    public void chunkCompleted() {
        int active = activeChunks.decrementAndGet();
        log.debug("Chunk processing completed. Active chunks: {}", active);
    }
}
```

### Deadlock Detection

```java
// JMX를 통한 Deadlock 모니터링
ThreadMXBean threadMXBean = ManagementFactory.getThreadMXBean();
long[] deadlockedThreads = threadMXBean.findDeadlockedThreads();

if (deadlockedThreads != null) {
    log.error("Deadlock detected! Thread IDs: {}", Arrays.toString(deadlockedThreads));
}
```

---

## 10. 테스트

### 동시성 테스트

```java
@SpringBootTest
class ConcurrencyTest {

    @Test
    void cacheInvalidate_동시호출_테스트() throws Exception {
        int threadCount = 10;
        CountDownLatch latch = new CountDownLatch(threadCount);

        ExecutorService executor = Executors.newFixedThreadPool(threadCount);

        for (int i = 0; i < threadCount; i++) {
            executor.submit(() -> {
                try {
                    cacheInvalidateClient.invalidateSafely("recruit");
                } finally {
                    latch.countDown();
                }
            });
        }

        latch.await(10, TimeUnit.SECONDS);

        // 실제로는 1번만 호출되어야 함
        verify(apiServer, times(1)).invalidateCache(any());
    }
}
```

---

## 관련 문서
- [Spring Batch 개발 가이드](./Spring_Batch_개발_가이드.md)
- [도메인 확장 가이드](./도메인_확장_가이드.md)

---

**최종 업데이트:** 2025-12-26
**패턴:** Spring Batch 6.0 ItemReader/Processor/Writer + Virtual Thread 병렬 테이블 쓰기
