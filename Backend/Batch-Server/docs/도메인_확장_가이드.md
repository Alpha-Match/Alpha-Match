...
**기존 ChunkProcessor를 참고하여 CompanyChunkProcessor 작성:**

```java
package com.alpha.backend.application.processor;

import com.alpha.backend.domain.company.*;
...
public class CompanyChunkProcessor {

    private final CompanyMetadataRepository metadataRepository;
    private final CompanyEmbeddingRepository embeddingRepository;

    @Transactional
    public void processChunk(List<CompanyRowData> chunk) {
        // 1. Metadata 저장
        List<CompanyMetadataEntity> metadataList = chunk.stream()
                .map(this::toMetadataEntity)
                .toList();
        metadataRepository.upsertAll(metadataList);

        // 2. Embedding 저장
        List<CompanyEmbeddingEntity> embeddingList = chunk.stream()
                .map(this::toEmbeddingEntity)
                .toList();
        embeddingRepository.upsertAll(embeddingList);

        log.info("Company chunk processed: {} rows", chunk.size());
    }

    private CompanyMetadataEntity toMetadataEntity(CompanyRowData row) {
        return CompanyMetadataEntity.builder()
                .id(row.getId())
                .name(row.getName())
    ...
    private CompanyEmbeddingEntity toEmbeddingEntity(CompanyRowData row) {
        return CompanyEmbeddingEntity.fromFloatArray(row.getId(), row.getVector());
    }
}
```
...
### 8.3 서비스 레이어
- [ ] {Domain}ChunkProcessor 작성
- [ ] {Domain}StreamingService 작성 (필요시)
- [ ] 기존 Router/Factory 패턴에 도메인 추가
