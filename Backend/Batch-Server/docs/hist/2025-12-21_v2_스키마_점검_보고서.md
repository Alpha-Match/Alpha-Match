# v2 스키마 업데이트 점검 보고서

**작성일**: 2025-12-21
**점검자**: AI Agent
**점검 범위**: Flyway V2 마이그레이션, 엔티티 v2, Repository 계층

---

## 📋 점검 개요

### 점검 대상

1. ✅ Flyway V2 마이그레이션 SQL 파일
2. ✅ 엔티티 11개 (v2 스키마 기준)
3. ✅ Repository 계층 (Domain + Infrastructure)
4. ✅ 벡터 차원 일관성
5. ✅ FK 제약조건 및 CASCADE 설정
6. ✅ 복합 PK 구현
7. ✅ UNIQUE 제약조건

### 점검 결과 요약

| 항목 | 상태 | 비고 |
|------|------|------|
| Flyway V2 SQL 문법 | 🟢 정상 | 일부 인덱스명 확인 필요 |
| 엔티티 11개 생성 | 🟢 정상 | - |
| 벡터 차원 일관성 | 🟢 정상 | 모두 384d |
| 복합 PK 구현 | 🟢 정상 | CandidateSkillId, RecruitSkillId |
| **Repository 누락** | 🔴 **심각** | **12개 누락** |
| UUID 생성 전략 | 🟡 검토 필요 | GenerationType.AUTO vs gen_random_uuid() |

---

## 🔴 심각한 문제 (즉시 수정 필요)

### 1. Repository 12개 완전 누락

엔티티는 생성했지만 Repository 계층이 구현되지 않았습니다.

#### 1.1 Recruit Domain (8개 누락)

**Domain Layer (Port - 인터페이스):**
```
❌ src/main/java/com/alpha/backend/domain/recruit/repository/RecruitRepository.java
❌ src/main/java/com/alpha/backend/domain/recruit/repository/RecruitDescriptionRepository.java
❌ src/main/java/com/alpha/backend/domain/recruit/repository/RecruitSkillRepository.java
❌ src/main/java/com/alpha/backend/domain/recruit/repository/RecruitSkillsEmbeddingRepository.java
```

**Infrastructure Layer (Adapter - JPA 구현체):**
```
❌ src/main/java/com/alpha/backend/infrastructure/persistence/RecruitJpaRepository.java
❌ src/main/java/com/alpha/backend/infrastructure/persistence/RecruitDescriptionJpaRepository.java
❌ src/main/java/com/alpha/backend/infrastructure/persistence/RecruitSkillJpaRepository.java
❌ src/main/java/com/alpha/backend/infrastructure/persistence/RecruitSkillsEmbeddingJpaRepository.java
```

**영향:**
- Recruit 도메인 데이터를 DB에 저장/조회 불가
- Batch Job에서 RecruitItemWriter 구현 불가
- gRPC로 받은 데이터를 처리할 수 없음

**우선순위:** 🔥 최우선

---

#### 1.2 SkillCategoryDic Domain (2개 누락)

**Domain Layer:**
```
❌ src/main/java/com/alpha/backend/domain/skilldic/repository/SkillCategoryDicRepository.java
```

**Infrastructure Layer:**
```
❌ src/main/java/com/alpha/backend/infrastructure/persistence/SkillCategoryDicJpaRepository.java
```

**영향:**
- 기술 스택 카테고리 정보 저장/조회 불가
- skill_embedding_dic의 category_id FK 참조 불가

**우선순위:** 🔥 최우선

---

#### 1.3 CandidateDescription Domain (2개 누락)

**Domain Layer:**
```
❌ src/main/java/com/alpha/backend/domain/candidate/repository/CandidateDescriptionRepository.java
```

**Infrastructure Layer:**
```
❌ src/main/java/com/alpha/backend/infrastructure/persistence/CandidateDescriptionJpaRepository.java
```

**영향:**
- 지원자 이력서 원문(Markdown) 저장 불가
- API 서버에서 상세 이력서 조회 불가

**우선순위:** 🔥 최우선

---

### 2. 기존 v1 Repository와의 충돌 가능성

**문제:**
현재 Infrastructure 계층에 v1용 Repository가 남아있습니다:
- `RecruitMetadataJpaRepository.java` (v1 - recruit_metadata 테이블용)
- `RecruitEmbeddingJpaRepository.java` (v1 - recruit_embedding 테이블용)

v2에서는 이 테이블들이 존재하지 않으므로 **런타임 오류 발생 가능**.

**해결 방안:**
1. v1 Repository 삭제 (권장)
2. 또는 `@Profile("v1")` 등으로 비활성화

**우선순위:** 🔥 높음

---

## 🟡 중요한 문제 (검토 필요)

### 3. UUID 생성 전략 불일치

**문제:**

**SkillCategoryDicEntity.java (Line 30-32):**
```java
@Id
@GeneratedValue(strategy = GenerationType.AUTO)
@Column(name = "category_id", updatable = false, nullable = false)
private UUID categoryId;
```

**Flyway V2 SQL (Line 38-39):**
```sql
CREATE TABLE skill_category_dic (
    category_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
```

**불일치 내용:**
- JPA: `GenerationType.AUTO` → Hibernate의 UUID 생성 전략 사용
- SQL: `DEFAULT gen_random_uuid()` → PostgreSQL 함수 사용

**잠재적 문제:**
- Hibernate가 SQL의 DEFAULT를 무시하고 자체 UUID 생성 가능
- 또는 INSERT 시 NULL이 들어가서 PostgreSQL이 gen_random_uuid() 호출
- 일관성 문제 발생 가능

**권장 수정:**

**방법 1: JPA 방식 통일 (권장)**
```java
@Id
@GeneratedValue(generator = "UUID")
@GenericGenerator(
    name = "UUID",
    strategy = "org.hibernate.id.UUIDGenerator"
)
@Column(name = "category_id")
private UUID categoryId;
```

```sql
-- SQL에서 DEFAULT 제거
category_id UUID PRIMARY KEY,
```

**방법 2: SQL DEFAULT 활용**
```java
@Id
@Column(name = "category_id")
private UUID categoryId;
// @GeneratedValue 제거, INSERT 전 수동 설정 또는 DB DEFAULT 활용
```

**영향 범위:**
- `SkillCategoryDicEntity.java`
- `SkillEmbeddingDicEntity.java`

**우선순위:** 🟡 중간 (테스트 시 확인 필요)

---

### 4. Flyway V2 인덱스 DROP 검증 필요

**문제:**

**V2__restructure_schema_to_v2.sql (Line 14-20):**
```sql
-- Drop Indexes
DROP INDEX IF EXISTS idx_candidate_skills_vector;
DROP INDEX IF EXISTS idx_skill_vector;
DROP INDEX IF EXISTS idx_recruit_vector;  -- ⚠️ 인덱스명 불일치 가능
DROP INDEX IF EXISTS idx_candidate_position_category;
DROP INDEX IF EXISTS idx_candidate_experience_years;
DROP INDEX IF EXISTS idx_recruit_primary_keyword;
DROP INDEX IF EXISTS idx_recruit_exp_years;
```

**V1 파일에서 실제 생성된 인덱스명 확인 필요:**
- `idx_recruit_vector` → 실제로는 `idx_recruit_embedding_vector`일 가능성
- `idx_recruit_exp_years` → 실제로는 `idx_recruit_metadata_exp_years`일 가능성

**확인 방법:**
```bash
# V1 파일 확인
grep "CREATE INDEX" Backend/Batch-Server/src/main/resources/db/migration/V1__init_database_schema.sql
```

**영향:**
- 인덱스명 불일치 시 DROP 실패 (하지만 `IF EXISTS` 사용으로 오류는 안남)
- 기존 인덱스가 남아있을 수 있음 (성능 영향 미미)

**우선순위:** 🟡 낮음 (기능적 문제 없음)

---

## 🟢 정상 확인된 사항

### 1. 벡터 차원 일관성 ✅

**확인 항목:**
- 모든 엔티티: `VECTOR_DIMENSION = 384`
- 모든 SQL: `VECTOR(384)`

**확인 결과:**

| 파일 | 라인 | 내용 | 상태 |
|------|------|------|------|
| CandidateSkillsEmbeddingEntity.java | 31 | `VECTOR_DIMENSION = 384` | ✅ |
| RecruitSkillsEmbeddingEntity.java | 31 | `VECTOR_DIMENSION = 384` | ✅ |
| SkillEmbeddingDicEntity.java | 32 | `VECTOR_DIMENSION = 384` | ✅ |
| V2 SQL | 53, 129, 143, 211, 224 | `VECTOR(384)` | ✅ |

**결론:** 모든 벡터 차원이 384로 통일됨. 문제 없음.

---

### 2. 복합 PK 구현 ✅

**CandidateSkillId.java:**
```java
@Data
@NoArgsConstructor
@AllArgsConstructor
@EqualsAndHashCode
public class CandidateSkillId implements Serializable {
    private static final long serialVersionUID = 1L;

    private UUID candidateId;
    private String skill;
}
```

**CandidateSkillEntity.java:**
```java
@Entity
@Table(name = "candidate_skill")
@IdClass(CandidateSkillId.class)
public class CandidateSkillEntity {
    @Id
    @Column(name = "candidate_id")
    private UUID candidateId;

    @Id
    @Column(name = "skill")
    private String skill;
}
```

**RecruitSkillId.java, RecruitSkillEntity.java:**
- 동일한 패턴으로 정상 구현됨

**SQL:**
```sql
PRIMARY KEY (candidate_id, skill)
PRIMARY KEY (recruit_id, skill)
```

**결론:** 복합 PK 구현 정상. JPA `@IdClass` 패턴 올바름.

---

### 3. UNIQUE 제약조건 ✅

**SkillCategoryDicEntity.java (Line 34):**
```java
@Column(name = "category", nullable = false, unique = true, columnDefinition = "TEXT")
private String category;
```

**SkillEmbeddingDicEntity.java (Line 42):**
```java
@Column(name = "skill", nullable = false, unique = true, columnDefinition = "TEXT")
private String skill;
```

**SQL (Line 39, 52):**
```sql
category TEXT NOT NULL UNIQUE,
skill TEXT NOT NULL UNIQUE,
```

**결론:** UNIQUE 제약조건 정상. Entity와 SQL 일치.

---

### 4. 타임스탬프 타입 매핑 ✅

**SQL:** `TIMESTAMPTZ` (PostgreSQL의 timezone-aware timestamp)
**Entity:** `OffsetDateTime` (Java의 timezone-aware timestamp)

**예시:**
```java
@CreationTimestamp
@Column(name = "created_at", updatable = false, nullable = false)
private OffsetDateTime createdAt;

@UpdateTimestamp
@Column(name = "updated_at", nullable = false)
private OffsetDateTime updatedAt;
```

**매핑 테이블:**

| PostgreSQL | Java | Hibernate 매핑 |
|------------|------|----------------|
| TIMESTAMPTZ | OffsetDateTime | ✅ 정상 |
| TIMESTAMP | LocalDateTime | - |

**결론:** 타임스탬프 매핑 정상. `@CreationTimestamp`, `@UpdateTimestamp` 사용으로 자동 관리.

---

### 5. FK 제약조건 (단방향 ID 참조 패턴) ✅

**현재 패턴:**
- Entity에는 UUID만 저장 (예: `private UUID recruitId;`)
- JPA 관계 매핑 없음 (`@OneToOne`, `@ManyToOne` 미사용)
- SQL에만 FK 제약조건 존재

**예시:**

**RecruitDescriptionEntity.java:**
```java
@Id
@Column(name = "recruit_id")
private UUID recruitId;
// ❌ @OneToOne 관계 없음 (의도된 설계)
```

**SQL:**
```sql
CONSTRAINT fk_recruit_description
    FOREIGN KEY (recruit_id)
    REFERENCES recruit(recruit_id)
    ON DELETE CASCADE
```

**설계 의도:**
- 단방향 ID 참조로 복잡도 감소
- N+1 문제 방지
- FK 제약조건은 DB 레벨에서만 관리

**결론:** 이것은 **의도된 설계 패턴**. 문제 없음.

---

### 6. CASCADE DELETE 설정 ✅

**SQL에서 CASCADE DELETE 확인:**

```sql
-- candidate → candidate_description (Line 93)
CONSTRAINT fk_candidate_description
    FOREIGN KEY (candidate_id)
    REFERENCES candidate(candidate_id)
    ON DELETE CASCADE

-- candidate → candidate_skill (Line 124)
CONSTRAINT fk_candidate_skill_candidate
    FOREIGN KEY (candidate_id)
    REFERENCES candidate(candidate_id)
    ON DELETE CASCADE

-- candidate → candidate_skills_embedding (Line 146)
CONSTRAINT fk_candidate_skills_embedding
    FOREIGN KEY (candidate_id)
    REFERENCES candidate(candidate_id)
    ON DELETE CASCADE

-- recruit → recruit_description (Line 179)
CONSTRAINT fk_recruit_description
    FOREIGN KEY (recruit_id)
    REFERENCES recruit(recruit_id)
    ON DELETE CASCADE

-- recruit → recruit_skill (Line 202)
CONSTRAINT fk_recruit_skill_recruit
    FOREIGN KEY (recruit_id)
    REFERENCES recruit(recruit_id)
    ON DELETE CASCADE

-- recruit → recruit_skills_embedding (Line 225)
CONSTRAINT fk_recruit_skills_embedding
    FOREIGN KEY (recruit_id)
    REFERENCES recruit(recruit_id)
    ON DELETE CASCADE
```

**DDD Aggregate 패턴:**
- `candidate` (Aggregate Root) → 하위 3개 테이블 CASCADE DELETE ✅
- `recruit` (Aggregate Root) → 하위 3개 테이블 CASCADE DELETE ✅

**결론:** CASCADE DELETE 설정 정상. DDD Aggregate 패턴 준수.

---

### 7. CHECK 제약조건 ✅

**SQL:**
```sql
-- candidate (Line 76)
experience_years INTEGER CHECK (experience_years > 0),

-- recruit (Line 153)
experience_years INTEGER CHECK (experience_years > 0),
```

**의미:**
- NULL 허용 (신입/경력무관)
- 값이 있을 경우 반드시 1 이상

**Entity 검증:**
```java
@Column(name = "experience_years")
private Integer experienceYears; // NULL 가능
```

**결론:** CHECK 제약조건 정상. NULL 허용 정책 일치.

---

## 📊 통계

### 파일 생성 현황

| 구분 | 생성됨 | 누락됨 | 합계 |
|------|--------|--------|------|
| 엔티티 | 11 | 0 | 11 |
| Domain Repository | 7 | 5 | 12 |
| JPA Repository | 5 | 7 | 12 |
| **총계** | **23** | **12** | **35** |

### 도메인별 Repository 현황

| 도메인 | 엔티티 | Domain Repo | JPA Repo | 상태 |
|--------|--------|-------------|----------|------|
| Recruit | 5 | 1 (v1) | 2 (v1) | 🔴 v2 미구현 |
| Candidate | 5 | 4 | 4 | 🟡 1개 누락 |
| SkillDic | 2 | 1 | 1 | 🟡 1개 누락 |
| Checkpoint | 1 | 1 | 1 | ✅ 완료 |
| DLQ | 1 | 1 | 1 | ✅ 완료 |

---

## 🔧 수정 필요 사항 (우선순위별)

### 🔥 최우선 (즉시 수정)

#### 1. Recruit Domain v2 Repository 8개 생성

**작업 목록:**

**Domain Layer:**
1. `RecruitRepository.java`
   - `void upsert(RecruitEntity entity);`
   - `void upsertAll(List<RecruitEntity> entities);`

2. `RecruitDescriptionRepository.java`
   - `void upsert(RecruitDescriptionEntity entity);`
   - `void upsertAll(List<RecruitDescriptionEntity> entities);`

3. `RecruitSkillRepository.java`
   - `void upsert(RecruitSkillEntity entity);`
   - `void upsertAll(List<RecruitSkillEntity> entities);`

4. `RecruitSkillsEmbeddingRepository.java`
   - `void upsert(RecruitSkillsEmbeddingEntity entity);`
   - `void upsertAll(List<RecruitSkillsEmbeddingEntity> entities);`

**Infrastructure Layer:**
1. `RecruitJpaRepository.java`
   - Native Query Upsert 구현

2. `RecruitDescriptionJpaRepository.java`
   - Native Query Upsert 구현

3. `RecruitSkillJpaRepository.java`
   - Native Query Upsert 구현 (복합 PK)

4. `RecruitSkillsEmbeddingJpaRepository.java`
   - Native Query Upsert 구현

**예시 코드 (RecruitJpaRepository):**
```java
@Repository
public interface RecruitJpaRepository
        extends JpaRepository<RecruitEntity, UUID>, RecruitRepository {

    @Override
    @Modifying
    @Query(value = """
        INSERT INTO recruit (
            recruit_id, position, company_name, experience_years,
            primary_keyword, english_level, published_at, updated_at
        )
        VALUES (
            :#{#entity.recruitId}, :#{#entity.position}, :#{#entity.companyName},
            :#{#entity.experienceYears}, :#{#entity.primaryKeyword},
            :#{#entity.englishLevel}, :#{#entity.publishedAt}, NOW()
        )
        ON CONFLICT (recruit_id)
        DO UPDATE SET
            position = EXCLUDED.position,
            company_name = EXCLUDED.company_name,
            experience_years = EXCLUDED.experience_years,
            primary_keyword = EXCLUDED.primary_keyword,
            english_level = EXCLUDED.english_level,
            published_at = EXCLUDED.published_at,
            updated_at = NOW()
        """, nativeQuery = true)
    void upsert(@Param("entity") RecruitEntity entity);

    @Override
    default void upsertAll(List<RecruitEntity> entities) {
        entities.forEach(this::upsert);
    }
}
```

---

#### 2. SkillCategoryDic Repository 2개 생성

**작업 목록:**

**Domain Layer:**
```java
// SkillCategoryDicRepository.java
public interface SkillCategoryDicRepository {
    Optional<SkillCategoryDicEntity> findByCategory(String category);
    void upsert(SkillCategoryDicEntity entity);
    void upsertAll(List<SkillCategoryDicEntity> entities);
}
```

**Infrastructure Layer:**
```java
// SkillCategoryDicJpaRepository.java
@Repository
public interface SkillCategoryDicJpaRepository
        extends JpaRepository<SkillCategoryDicEntity, UUID>,
                SkillCategoryDicRepository {

    @Override
    Optional<SkillCategoryDicEntity> findByCategory(String category);

    @Override
    @Modifying
    @Query(value = """
        INSERT INTO skill_category_dic (category, updated_at)
        VALUES (:#{#entity.category}, NOW())
        ON CONFLICT (category)
        DO UPDATE SET
            updated_at = NOW()
        RETURNING category_id
        """, nativeQuery = true)
    void upsert(@Param("entity") SkillCategoryDicEntity entity);

    @Override
    default void upsertAll(List<SkillCategoryDicEntity> entities) {
        entities.forEach(this::upsert);
    }
}
```

**주의사항:**
- `category_id`는 자동 생성되므로 INSERT 시 제외
- `RETURNING category_id`로 생성된 ID를 Entity에 반영할지 검토 필요

---

#### 3. CandidateDescription Repository 2개 생성

**작업 목록:**

**Domain Layer:**
```java
// CandidateDescriptionRepository.java
public interface CandidateDescriptionRepository {
    void upsert(CandidateDescriptionEntity entity);
    void upsertAll(List<CandidateDescriptionEntity> entities);
}
```

**Infrastructure Layer:**
```java
// CandidateDescriptionJpaRepository.java
@Repository
public interface CandidateDescriptionJpaRepository
        extends JpaRepository<CandidateDescriptionEntity, UUID>,
                CandidateDescriptionRepository {

    @Override
    @Modifying
    @Query(value = """
        INSERT INTO candidate_description (
            candidate_id, original_resume, resume_lang, updated_at
        )
        VALUES (
            :#{#entity.candidateId}, :#{#entity.originalResume},
            :#{#entity.resumeLang}, NOW()
        )
        ON CONFLICT (candidate_id)
        DO UPDATE SET
            original_resume = EXCLUDED.original_resume,
            resume_lang = EXCLUDED.resume_lang,
            updated_at = NOW()
        """, nativeQuery = true)
    void upsert(@Param("entity") CandidateDescriptionEntity entity);

    @Override
    default void upsertAll(List<CandidateDescriptionEntity> entities) {
        entities.forEach(this::upsert);
    }
}
```

---

#### 4. v1 Repository 삭제 또는 비활성화

**삭제 대상:**
- `RecruitMetadataRepository.java` (Domain)
- `RecruitEmbeddingRepository.java` (Domain)
- `RecruitMetadataJpaRepository.java` (Infrastructure)
- `RecruitEmbeddingJpaRepository.java` (Infrastructure)

**방법 1: 삭제 (권장)**
```bash
rm Backend/Batch-Server/src/main/java/com/alpha/backend/domain/recruit/repository/RecruitMetadataRepository.java
rm Backend/Batch-Server/src/main/java/com/alpha/backend/domain/recruit/repository/RecruitEmbeddingRepository.java
rm Backend/Batch-Server/src/main/java/com/alpha/backend/infrastructure/persistence/RecruitMetadataJpaRepository.java
rm Backend/Batch-Server/src/main/java/com/alpha/backend/infrastructure/persistence/RecruitEmbeddingJpaRepository.java
```

**방법 2: 비활성화 (v1 롤백 대비)**
```java
@Repository
@Profile("v1") // v1 프로파일에서만 활성화
public interface RecruitMetadataJpaRepository ... {
}
```

---

### 🟡 중간 우선순위

#### 5. UUID 생성 전략 통일

**대상:**
- `SkillCategoryDicEntity.java`
- `SkillEmbeddingDicEntity.java`

**수정 방법 1: Hibernate UUID Generator 사용 (권장)**

```java
import org.hibernate.annotations.GenericGenerator;

@Entity
@Table(name = "skill_category_dic")
public class SkillCategoryDicEntity {

    @Id
    @GeneratedValue(generator = "UUID")
    @GenericGenerator(
        name = "UUID",
        strategy = "org.hibernate.id.UUIDGenerator"
    )
    @Column(name = "category_id", updatable = false, nullable = false)
    private UUID categoryId;

    // ...
}
```

**Flyway V2 SQL 수정:**
```sql
CREATE TABLE skill_category_dic (
    category_id UUID PRIMARY KEY,  -- DEFAULT 제거
    category TEXT NOT NULL UNIQUE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
```

**수정 방법 2: SQL DEFAULT 활용**

```java
@Entity
@Table(name = "skill_category_dic")
public class SkillCategoryDicEntity {

    @Id
    @Column(name = "category_id", updatable = false, nullable = false)
    private UUID categoryId;
    // @GeneratedValue 제거

    // ...
}
```

SQL은 그대로 유지:
```sql
category_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
```

**권장:** 방법 1 (Hibernate 통합 관리)

---

#### 6. Flyway V1 인덱스명 확인 및 V2 수정

**작업:**
1. V1 파일에서 실제 인덱스명 확인
2. V2 DROP INDEX 구문 수정

**확인 명령:**
```bash
grep "CREATE INDEX" Backend/Batch-Server/src/main/resources/db/migration/V1__init_database_schema.sql
```

**예상 수정:**
```sql
-- V2 파일 수정
DROP INDEX IF EXISTS idx_recruit_embedding_vector;  -- 수정됨
DROP INDEX IF EXISTS idx_recruit_metadata_exp_years;  -- 수정됨
```

---

## 📋 다음 단계 권장사항

### Phase 1: Repository 생성 (필수)

1. **Recruit Domain Repository 8개** (1-2시간)
   - Domain 인터페이스 4개
   - JPA 구현체 4개 (Native Query Upsert)

2. **SkillCategoryDic Repository 2개** (30분)
   - Domain 인터페이스 1개
   - JPA 구현체 1개

3. **CandidateDescription Repository 2개** (30분)
   - Domain 인터페이스 1개
   - JPA 구현체 1개

4. **v1 Repository 삭제** (10분)
   - 4개 파일 삭제 또는 비활성화

**예상 소요 시간:** 3-4시간

---

### Phase 2: UUID 생성 전략 통일 (선택)

1. **Entity 수정** (30분)
   - SkillCategoryDicEntity
   - SkillEmbeddingDicEntity

2. **Flyway V2 수정** (10분)
   - DEFAULT 제거

**예상 소요 시간:** 40분

---

### Phase 3: Flyway V1 검증 및 V2 수정 (선택)

1. **V1 인덱스명 확인** (10분)
2. **V2 DROP INDEX 수정** (10분)

**예상 소요 시간:** 20분

---

### Phase 4: 통합 테스트

1. **Flyway 마이그레이션 테스트**
   ```bash
   ./gradlew flywayClean
   ./gradlew flywayMigrate
   ```

2. **애플리케이션 실행 테스트**
   ```bash
   ./gradlew bootRun
   ```

3. **Repository 동작 확인**
   - Upsert 테스트
   - FK 제약조건 검증
   - CASCADE DELETE 검증

**예상 소요 시간:** 1-2시간

---

## 📌 체크리스트

### 즉시 수정 필요 (Phase 1)

- [ ] RecruitRepository 생성 (Domain)
- [ ] RecruitDescriptionRepository 생성 (Domain)
- [ ] RecruitSkillRepository 생성 (Domain)
- [ ] RecruitSkillsEmbeddingRepository 생성 (Domain)
- [ ] RecruitJpaRepository 생성 (Infrastructure)
- [ ] RecruitDescriptionJpaRepository 생성 (Infrastructure)
- [ ] RecruitSkillJpaRepository 생성 (Infrastructure)
- [ ] RecruitSkillsEmbeddingJpaRepository 생성 (Infrastructure)
- [ ] SkillCategoryDicRepository 생성 (Domain)
- [ ] SkillCategoryDicJpaRepository 생성 (Infrastructure)
- [ ] CandidateDescriptionRepository 생성 (Domain)
- [ ] CandidateDescriptionJpaRepository 생성 (Infrastructure)
- [ ] v1 Repository 삭제/비활성화 (4개)

### 검토 필요 (Phase 2-3)

- [ ] UUID 생성 전략 통일 (SkillCategoryDicEntity, SkillEmbeddingDicEntity)
- [ ] Flyway V1 인덱스명 확인
- [ ] Flyway V2 DROP INDEX 수정

### 테스트 (Phase 4)

- [ ] Flyway Clean & Migrate 성공
- [ ] 애플리케이션 실행 성공
- [ ] Repository Upsert 동작 확인
- [ ] FK 제약조건 검증
- [ ] CASCADE DELETE 동작 확인

---

## 📝 참고 문서

- **테이블 명세서**: `/Backend/docs/table_specification.md`
- **DB 스키마 가이드**: `/Backend/docs/DB_스키마_가이드.md`
- **도메인 확장 가이드**: `/Backend/Batch-Server/docs/도메인_확장_가이드.md`
- **Flyway 마이그레이션 가이드**: `/Backend/docs/Flyway_마이그레이션_가이드.md`
- **v2 테스트 세팅 가이드**: `/docs/v2_테스트_세팅_가이드.md`

---

**점검 완료일**: 2025-12-21
**다음 검토 예정**: Repository 12개 생성 완료 후
